	...
	return (
		<div { ...useBlockProps() }>
			...
			<figure className="icon">
				<img src="/wp-content/plugins/alda-weather/weather-icons/svg/wi-day-cloudy.svg" alt="Cloudy" />
			</figure>
			<div className="content">
				<p className="station-name">{ attributes.stationName }</p>
				<ul className="stats">
					<li className="stat">
						<span className="stat-label">
							{ __( 'Temperature:', 'alda-weather' ) }{ ' ' }
						</span>
						{ convertTempratureUnit(
							attributes.temperatureUnit,
							17.41
						) }
						{ mapTemperatureUnit( attributes.temperatureUnit ) }
					</li>
					<li className="stat">
						<span className="stat-label">
							{ __( 'Wind Speed:', 'alda-weather' ) }{ ' ' }
						</span>
						{ convertWindSpeedUnit( attributes.windSpeedUnit, 5 ) }{ ' ' }
						{ mapWindSpeedUnit( attributes.windSpeedUnit ) }
					</li>
					<li className="stat">{ __( 'Overcast' ) }</li>
				</ul>
			</div>
		</div>
	);
